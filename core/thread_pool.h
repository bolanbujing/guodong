...
public:
    ...
    explicit ThreadPool(const std::string& threadName, size_t size)
        ...
        std::shared_ptr<std::vector<std::string>> fields = std::make_shared<std::vector<std::string>>();
        if (boost::log::core::get()) {
            boost::split(*fields, thread_name_, boost::is_any_of("-"), boost::token_compress_on);
        }
