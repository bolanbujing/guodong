...
TcpServer::TcpServer(boost::asio::io_context& io_context) : io_context_(io_context),
...
void TcpServer::startAccept() {
    ...
    acceptor_.async_accept(session->Socket(), boost::bind(&TcpServer::handleAccept, this, session, boost::asio::placeholders::error));
...
void TcpServer::handleAccept(TcpSession::Ptr session, const boost::system::error_code& error) {
    if (!error) {
        ...
        std::function<void(TcpSession::Ptr, int, Message::Ptr)> fp = std::bind(&TcpServer::onClientRequest, this,
...
void TcpServer::onClientRequest(TcpSession::Ptr, int error, Message::Ptr message) {
    ...
    LOG(TRACE) << LOG_KV("error", error) << LOG_KV("body", content)
    << LOG_KV("type", type) << LOG_KV("askid", askid);
}